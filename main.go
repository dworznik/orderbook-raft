...
import (
	...
	"github.com/dworznik/orderbook-raft/server"
...
func main() {
	...
	conf := config{
		Server: configServer{
			...
			Addr: v.GetString(serverAddr),
		},
		Raft: configRaft{
			NodeId:    v.GetString(raftNodeId),
			Port:      v.GetInt(raftPort),
			Addr:      v.GetString(raftAddr),
	...
	fsmStore := fsm.NewOrderbookFSM(*ob)
	...
	srv := server.New(fmt.Sprintf(":%d", conf.Server.Port), ob, raftServer)
